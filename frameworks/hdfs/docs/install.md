...
Clients connecting to HDFS over a TLS connection must connect to HTTPS specific port. Each node type (`journal`, `name` and `data`) can be configured with different port number for TLS connections.
